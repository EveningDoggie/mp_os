    ...
    size_t values_count)
{
    ...
    if (current_block == nullptr) //нет ни одного зан€того блока
    {
    ...
    while (current_block != nullptr) //на последней итерации будет проверено наличие свободной €чейки перед концом пам€ти
    {
        previous_block = current_block;
        current_block = get_occupied_block_next_block_ptr(current_block);

        between_block_size = //если есть след блок то отнимаем от него. иначе от конца пам€ти
    ...
    if (size_optimal==-1 || size_optimal==get_space_size()+1) //подход€щих блоков не найдено
    {
    ...
    if (right_block_size <= get_occupied_block_minimum_size()) //отдать весь блок, если размер правого блока = 0 или меньше допустимого (мета+мин«нач - оптимизаци€ из кнута)
    {
    ...
    void *at)
{
    ...
    // автоматом проставл€етс€ nullptr если это был последний зан€тый (хранитс€ в next block). ниже аналогично
    ...
    //свободные блоки считаютс€ как промежуток между зан€тыми, поэтому "объединение" как в прошлом делать не нужно
...
{
    //аллокатор, логгер, размер кучи, мьютекс, фитмод, указатель на 1 элемент, размер свободной пам€ти
...
{
    return sizeof(size_t) + sizeof(void*) * 3; //размер блока, начало пам€ти, указатель на след зан€тый, указатель на предыдущий зан€тый
}
...
inline size_t allocator_boundary_tags::get_occupied_block_minimum_size() const
{
    ...
    return get_occupied_block_metadata_size() + 8; //минимальный кусок должен включать хоть сколько то малое пространство (оптимизаци€ из кнута: обычно берутс€ значени€ от 8 до 10 б)
}
...
std::vector<allocator_test_utils::block_info> allocator_boundary_tags::get_blocks_info() const noexcept
{
    ...
    if (current_block == nullptr) //если нет зан€тых
    {
