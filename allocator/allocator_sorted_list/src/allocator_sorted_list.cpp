...
size_t allocator_sorted_list::get_free_block_minimum_size() const
{
    ...
    return get_free_block_metadata_size() + 8; //минимальный кусок должен включать метаданные и хоть сколько то малое пространство (оптимизаци€ из кнута: обычно берутс€ значени€ от 8 до 10 б)
}
